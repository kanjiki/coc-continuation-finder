...
function card(e){
  ...
  return `<article class="card${caution?' caution':''}"><div class="cardtop"><h3>${esc(e.t)}</h3><span class="status-dot ${caution?'warn':'good'}"></span></div><div class="badges"><span class="badge ${caution?'warn':'good'}">${esc(e.st)}</span><span class="badge">${esc(e.c)}</span><span class="badge">${esc(e.e)}</span></div>${intro}${markets}</article>`
}
